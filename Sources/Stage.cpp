...
bool Stage::IsGoal(float playerX) {
	...
	return playerX >= goalPosition[GameBase::Instance().GetStageNumber()] + CLEARCANGE_POS;
}
...
void Stage::Initialization() {
	...
	backDrawPos = VGet(DRAW_BACKSTAGE_X, 0, DRAW_BACKSTAGE_Z[GameBase::Instance().GetStageNumber()]);
}

void Stage::DrawBackStage(Player player) {
	// 車が動いているように見せるためにステージ背景を左側に動かす
	if (!GameBase::Instance().GetIsGameStop()) {
		backDrawPos.x += 1.5f;
		if (player.GetPosition().x > goalPosition[GameBase::Instance().GetStageNumber()]) {
			backDrawPos.x -= 8;
		}
	}
