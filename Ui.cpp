...
void SquareTest(int loopY, int loopX, VECTOR topLeft, int lengthX, int lengthY, int plusX, int plusY, SCREEN_TYPE screen, bool isSquare) {
	float posX = topLeft.x;
	float posY = topLeft.y - plusY;
	for (int y = 0; y < loopY; y++) {	// 描画する座標を変更しながら図形を描画(形が等しく位置も等間隔にしか置けない）
		posX = topLeft.x;
		posY += (float)plusY;
		for (int x = 0; x < loopX; x++) {
			if (isSquare) {
				DrawBox((int)ScreenDrawPos(screenWidth, posX), (int)ScreenDrawPos(screenHeight, posY), (int)ScreenDrawPos(screenWidth, posX + lengthX), (int)ScreenDrawPos(screenHeight, posY + lengthY), buttonMap[screen][y][x] == 2 ? COLOR_GREEN : COLOR_GRAY, TRUE);
			}
			else {
				DrawTriangleAA(ScreenDrawPos(screenWidth, posX), ScreenDrawPos(screenHeight, posY), ScreenDrawPos(screenWidth, posX + lengthX), ScreenDrawPos(screenHeight, posY), ScreenDrawPos(screenWidth, posX + lengthX), ScreenDrawPos(screenHeight, posY + lengthY), buttonMap[screen][y][x] == 2 ? COLOR_GREEN : COLOR_GRAY, TRUE);
			}
			posX += plusX;
		}
	}
...
void StringTest(std::string drawText, int boxLeftPos, int boxRightPos, int drawPosY, int font, bool isNum, int num) {
	...
	if (isNum) {
		DrawFormatStringToHandle(drawPosX, (int)ScreenDrawPos(screenHeight, (float)drawPosY), COLOR_BLACK, font, "%06d", num);
	}
	else {
		DrawFormatStringToHandle(drawPosX, (int)ScreenDrawPos(screenHeight, (float)drawPosY), COLOR_BLACK, font, "%s", const_cast<char*>(drawText.c_str()));
	}
}
...
void DrawUI()
{
	...
	DrawStringToHandle((int)ScreenDrawPos(screenWidth, 80), (int)ScreenDrawPos(screenWidth, 95), "Ver_0.0.00.00", COLOR_BLACK, smallFontHandle);
	break;
	case STAGESELECT:
	{
		stageSelect1.Draw();
	...
	case INGAME:
		// インゲームのUIを用意したらここに配置
		DrawFormatStringToHandle((int)ScreenDrawPos(screenWidth, 1), (int)ScreenDrawPos(screenHeight, 95), COLOR_BLACK, smallFontHandle, "SCORE:%06d", inGameVewScore);
		if (currentScreenType == PAUSE) {
			DrawBox((int)ScreenDrawPos(screenWidth, 25), (int)ScreenDrawPos(screenHeight, 25), (int)ScreenDrawPos(screenWidth, 75), (int)ScreenDrawPos(screenHeight, 75), COLOR_WHITE_GRAY, TRUE);
			{
				resumeGame.Draw();
				pauseGameExit.Draw();
			}
		...
		if (currentScreenType == GAMEOVER) {
			DrawBox((int)ScreenDrawPos(screenWidth, 23), (int)ScreenDrawPos(screenHeight, 18), (int)ScreenDrawPos(screenWidth, 77), (int)ScreenDrawPos(screenHeight, 82), COLOR_WHITE_GRAY, TRUE);
			{
...
void ScreenFade(float fadeSpeed)
{
	// SetDrawBlendMode設定し直すことでフェード画面のみの透明度を変更する
	SetDrawBlendMode(DX_BLENDMODE_ALPHA, alphaValue);
	alphaValue = (int)ClampNum((float)alphaValue + (float)fadeSpeed, 0, 255);
...
void DrawStartCountDown() {
	...
	if (alphaValue >= 255) {	// 文字がくっきり見えるようになったら一定時間待機した後フェードアウト速度を描画している文字に応じて変更
		alphaValue = 255;
		WaitTimer(300);
		fadeSpeed = drawText == START_COUNTDOWN_2 ? (int)(-FADE_SPEED * 2.5f) : (int)(-FADE_SPEED / 1.5f);
	}
	SetDrawBlendMode(DX_BLENDMODE_ALPHA, alphaValue);
...
void DrawProgressRateBar(const Player& player, float startPos, float endPos, float heightPos) {	// 画面の下をUI表示ゾーンにするためリファクタリングする(動きもかくかくなので何とかする
	...
	DrawBox((int)ScreenDrawPos(screenWidth, startPos), (int)ScreenDrawPos(screenHeight, heightPos - 1), (int)ScreenDrawPos(screenWidth, startPos + w), (int)ScreenDrawPos(screenHeight, heightPos + 1), GetColor(0, 200, 0), TRUE);
...
void DrawTextCenter(float left, float top, float right, float bottom, std::string text, int fontType) {
	...
	DrawStringToHandle((int)drawPosX, (int)drawPosY, const_cast<char*>(text.c_str()), COLOR_BLACK, font);
}